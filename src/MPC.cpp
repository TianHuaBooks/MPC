...
size_t delta_start = epsi_start + N; // delta and a have (N-1) elements
...
void reinit_offsets() {
     ...
     delta_start = epsi_start + N; // delta and a have (N-1) elements
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // `fg` a vector of the cost constraints, `vars` is a vector of variable values (state & actuators)
      ...
      // Miminize the gap of sequences of actuators
      for (auto i = 0; i < N-2; i++) {
          fg[0] += 200*CppAD::pow(vars[delta_start+i+1]-vars[delta_start+i], 2);
          fg[0] += 100*CppAD::pow(vars[a_start+i+1]-vars[a_start+i], 2);
      }
      
      // Setup constraints, init constraints
      ...
      // go thru the rest of teh constraints
      for (auto i = 1; i < N; i++) {
          ...
          fg[1+psi_start+i] = psi1 - (psi0 - v0 * delta * dt / Lf);
...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  if (ok) {
      // Return the first actuator values and projected points. The variables can be accessed with
      // `solution.x[i]`.
      //
      ...
      std::cout << "Cost " << cost << " delta:" << solution.x[delta_start] << std::endl;
  } else {
      result.push_back(0.0);
      result.push_back(0.0); // not to accelerate 
      for (auto i = 0; i < N-1; i++) {
          result.push_back(vars[x_start+i]);
          result.push_back(vars[y_start+i]);
      }
      std::cout << "***** CppAD::ipopt::solve Failed!" << std::endl;
  }
